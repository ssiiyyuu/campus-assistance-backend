...
import com.siyu.server.entity.vo.flowable.CampusReportVO;
import com.siyu.server.service.flowable.CampusReportService;
...
@Api(tags = "工作流——校园上报流程")
@RestController("/flowable/campusReport")
public class CampusReportController extends FlowableBaseController {
    ...
    public R<String> studentCreate(@RequestBody @Valid CampusReportVO.Create create) {
        ShiroUser user = ShiroUtils.getCurrentUser();
        String assigneeId = getAssignee(user, "系级辅导员", DepartmentLevel.DEPARTMENT).getId();
    ...
    public R<String> departmentCounselorDelegate(@PathVariable String taskId, @PathVariable String delegateId) {
        ShiroUser user = ShiroUtils.getCurrentUser();
        if(hasPermission(user, delegateId)) {
    ...
    public R<String> gradeCounselorHandover(@PathVariable String taskId, @PathVariable String delegateId, @Valid @RequestBody CommentDTO commentDTO) {
        ...
        if(belongTheSameDepartment(user, delegateId)) {
